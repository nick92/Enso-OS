...
static void ____lambda17_ (Block13Data* _data13_, GObject* src_obj, GAsyncResult* res);
...
static void ____lambda18__gfunc (gconstpointer data, gpointer self);
...
static gboolean synapse_data_sink_initialize_caches_co (SynapseDataSinkInitializeCachesData* _data_) {
	...
	synapse_dbus_service_initialize (_data_->_tmp1_, ___lambda13__gasync_ready_callback, block12_data_ref (_data_->_data12_));
	_data_->_tmp2_ = NULL;
	...
	synapse_desktop_file_service_initialize (_data_->_tmp4_, ___lambda14__gasync_ready_callback, block12_data_ref (_data_->_data12_));
	_data_->_state_ = 2;
	return FALSE;
	_state_2:
	;
...
static void ____lambda17_ (Block13Data* _data13_, GObject* src_obj, GAsyncResult* res) {
	...
	plugin = _tmp1_;
	{
		...
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			if (_inner_error_->domain == SYNAPSE_SEARCH_ERROR) {
				goto __catch31_synapse_search_error;
			}
			_g_object_unref0 (plugin);
	...
	goto __finally31;
	__catch31_synapse_search_error:
	{
		GError* err = NULL;
		GError* _tmp10_ = NULL;
		err = _inner_error_;
...
static void _____lambda17__gasync_ready_callback (GObject* source_object, GAsyncResult* res, gpointer self) {
	____lambda17_ (self, source_object, res);
...
static gboolean synapse_data_sink_real_search_co (SynapseDataSinkSearchData* _data_) {
	...
	_data_->_data13_->waiting = FALSE;
	{
		...
		while (TRUE) {
			...
			synapse_item_provider_search (_data_->_tmp44_, &_data_->_tmp45_, _____lambda17__gasync_ready_callback, block13_data_ref (_data_->_data13_));
	...
	if (_data_->_tmp46_ != NULL) {
		...
		g_cancellable_connect (_data_->_tmp47_, (GCallback) ____lambda18__gfunc, block13_data_ref (_data_->_data13_), block13_data_unref);
	}
