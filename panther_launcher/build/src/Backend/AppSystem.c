	...
	if (_tmp8_) {
		{
			...
			while (TRUE) {
				...
				filename = _tmp30_;
				_g_free0 (_tmp29_);
				_tmp31_ = self->priv->apps_menu;
				_tmp32_ = filename;
				_tmp33_ = gmenu_tree_get_entry_by_id (_tmp31_, _tmp32_);
				te = _tmp33_;
				_tmp34_ = te;
				_tmp35_ = panther_backend_app_new (_tmp34_);
				app = _tmp35_;
				_tmp36_ = _result_;
				_tmp37_ = app;
		...
		__catch11_g_error:
		{
			...
			g_critical ("AppSystem.vala:125: Error loading dock elements from '%s'. (%s)", _tmp38_, _tmp43_);
			_g_free0 (_tmp38_);
			_g_error_free0 (e);
		}
		__finally11:
