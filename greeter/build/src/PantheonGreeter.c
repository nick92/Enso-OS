...
PantheonGreeter* pantheon_greeter_construct (GType object_type) {
	...
	GtkClutterActor* _tmp123_ = NULL;
	ClutterActor* _tmp124_ = NULL;
	GtkClutterActor* _tmp125_ = NULL;
	ClutterActor* _tmp126_ = NULL;
	GtkClutterActor* _tmp127_ = NULL;
	ClutterActor* _tmp128_ = NULL;
	UserListActor* _tmp129_ = NULL;
	...
	GdkDisplay* _tmp144_ = NULL;
	Display* _tmp145_ = NULL;
	Display* _tmp146_ = NULL;
	gint _tmp147_ = 0;
	gint _tmp148_ = 0;
	gint _tmp149_ = 0;
	gint _tmp150_ = 0;
	Display* _tmp151_ = NULL;
	gint _tmp152_ = 0;
	LoginGateway* _tmp153_ = NULL;
	LoginGateway* _tmp154_ = NULL;
	gboolean _tmp155_ = FALSE;
	gboolean _tmp156_ = FALSE;
	gchar* select_user = NULL;
	LoginGateway* _tmp158_ = NULL;
	LoginGateway* _tmp159_ = NULL;
	...
	UserList* _tmp189_ = NULL;
	LoginOption* _tmp190_ = NULL;
	LoginOption* _tmp191_ = NULL;
	GtkStyleContext* _tmp196_ = NULL;
	GtkCssProvider* provider = NULL;
	GtkCssProvider* _tmp197_ = NULL;
	GtkCssProvider* _tmp198_ = NULL;
	GdkScreen* _tmp199_ = NULL;
	GtkCssProvider* _tmp200_ = NULL;
	UserListActor* _tmp201_ = NULL;
	ClutterPropertyTransition* _tmp202_ = NULL;
	ClutterPropertyTransition* _tmp203_ = NULL;
	GtkClutterActor* _tmp204_ = NULL;
	ClutterPropertyTransition* _tmp205_ = NULL;
	ClutterPropertyTransition* _tmp206_ = NULL;
	GtkClutterActor* _tmp207_ = NULL;
	ClutterPropertyTransition* _tmp208_ = NULL;
	ClutterPropertyTransition* _tmp209_ = NULL;
	GdkWindow* _tmp210_ = NULL;
	...
	if (_tmp17_) {
		...
		g_message ("PantheonGreeter.vala:92: Using dummy LightDM because LIGHTDM_TO_SERVER" \
"_FD was not found.");
	...
	_tmp41_ = state_dir;
	_tmp42_ = g_build_filename (_tmp41_, state_file_name, NULL);
	_g_free0 (self->priv->state_file);
	self->priv->state_file = _tmp42_;
	_tmp43_ = g_key_file_new ();
	...
	self->state = _tmp43_;
	{
		...
		_tmp44_ = self->state;
		_tmp45_ = self->priv->state_file;
		g_key_file_load_from_file (_tmp44_, _tmp45_, G_KEY_FILE_NONE, &_inner_error_);
	...
	__catch0_g_error:
	{
		...
		_inner_error_ = NULL;
		_tmp46_ = e;
		if (!g_error_matches (_tmp46_, G_FILE_ERROR, G_FILE_ERROR_NOENT)) {
			const gchar* _tmp47_ = NULL;
			GError* _tmp48_ = NULL;
			const gchar* _tmp49_ = NULL;
			_tmp47_ = self->priv->state_file;
			_tmp48_ = e;
			_tmp49_ = _tmp48_->message;
			g_warning ("PantheonGreeter.vala:112: Failed to load state from %s: %s\n", _tmp47_, _tmp49_);
		}
		_g_error_free0 (e);
	}
	__finally0:
	...
	self->settings = _tmp50_;
	{
		GKeyFile* _tmp51_ = NULL;
		gchar* _tmp52_ = NULL;
		gchar* _tmp53_ = NULL;
		_tmp51_ = self->settings;
		_tmp52_ = g_build_filename (CONSTANTS_CONF_DIR, "greeter.conf", NULL);
		_tmp53_ = _tmp52_;
		g_key_file_load_from_file (_tmp51_, _tmp53_, G_KEY_FILE_KEEP_COMMENTS, &_inner_error_);
		_g_free0 (_tmp53_);
		if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	__catch1_g_error:
	{
		...
		g_warning ("PantheonGreeter.vala:120: %s", _tmp55_);
		_g_error_free0 (e);
	}
	__finally1:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	_tmp56_ = (GtkClutterEmbed*) gtk_clutter_embed_new ();
	g_object_ref_sink (_tmp56_);
	_g_object_unref0 (self->priv->clutter);
	self->priv->clutter = _tmp56_;
	_tmp57_ = self->priv->clutter;
	gtk_widget_add_events ((GtkWidget*) _tmp57_, (gint) (GDK_BUTTON_RELEASE_MASK | GDK_BUTTON_PRESS_MASK));
	_tmp58_ = self->priv->clutter;
	_tmp59_ = gtk_clutter_embed_get_stage (_tmp58_);
	_tmp60_ = _g_object_ref0 (G_TYPE_CHECK_INSTANCE_TYPE (_tmp59_, clutter_stage_get_type ()) ? ((ClutterStage*) _tmp59_) : NULL);
	stage = _tmp60_;
	_tmp61_ = stage;
	_tmp62_.red = (guint8) 0;
	_tmp62_.green = (guint8) 0;
	_tmp62_.blue = (guint8) 0;
	_tmp62_.alpha = (guint8) 255;
	clutter_actor_set_background_color ((ClutterActor*) _tmp61_, &_tmp62_);
	_tmp63_ = lightdm_user_list_get_instance ();
	_tmp64_ = user_list_new (_tmp63_);
	_g_object_unref0 (self->priv->userlist);
	self->priv->userlist = _tmp64_;
	_tmp65_ = self->priv->userlist;
	_tmp66_ = user_list_actor_new (_tmp65_);
	g_object_ref_sink (_tmp66_);
	_g_object_unref0 (self->priv->userlist_actor);
	self->priv->userlist_actor = _tmp66_;
	_tmp67_ = self->priv->userlist_actor;
	clutter_actor_set_opacity ((ClutterActor*) _tmp67_, (guint8) 0);
	_tmp68_ = time_label_new ();
	g_object_ref_sink (_tmp68_);
	time_label = _tmp68_;
	_tmp69_ = (GtkClutterActor*) gtk_clutter_actor_new ();
	g_object_ref_sink (_tmp69_);
	_g_object_unref0 (self->priv->time_actor);
	self->priv->time_actor = _tmp69_;
	_tmp70_ = self->priv->time_actor;
	_tmp71_ = gtk_clutter_actor_get_widget (_tmp70_);
	_tmp72_ = time_label;
	gtk_container_add (G_TYPE_CHECK_INSTANCE_CAST (_tmp71_, gtk_container_get_type (), GtkContainer), (GtkWidget*) _tmp72_);
	_tmp73_ = power_label_new ();
	g_object_ref_sink (_tmp73_);
	power_label = _tmp73_;
	_tmp74_ = (GtkClutterActor*) gtk_clutter_actor_new ();
	g_object_ref_sink (_tmp74_);
	_g_object_unref0 (self->priv->power_actor);
	self->priv->power_actor = _tmp74_;
	_tmp75_ = self->priv->power_actor;
	_tmp76_ = gtk_clutter_actor_get_widget (_tmp75_);
	_tmp77_ = power_label;
	gtk_container_add (G_TYPE_CHECK_INSTANCE_CAST (_tmp76_, gtk_container_get_type (), GtkContainer), (GtkWidget*) _tmp77_);
	_tmp78_ = wallpaper_new ();
	g_object_ref_sink (_tmp78_);
	_g_object_unref0 (self->priv->wallpaper);
	self->priv->wallpaper = _tmp78_;
	_tmp79_ = (GtkClutterActor*) gtk_clutter_actor_new ();
	g_object_ref_sink (_tmp79_);
	_g_object_unref0 (self->priv->wallpaper_actor);
	self->priv->wallpaper_actor = _tmp79_;
	_tmp80_ = self->priv->wallpaper_actor;
	_tmp81_ = gtk_clutter_actor_get_widget (_tmp80_);
	_tmp82_ = self->priv->wallpaper;
	gtk_container_add (G_TYPE_CHECK_INSTANCE_CAST (_tmp81_, gtk_container_get_type (), GtkContainer), (GtkWidget*) _tmp82_);
	blur_effect = FALSE;
	{
		gboolean _tmp83_ = FALSE;
		GKeyFile* _tmp84_ = NULL;
		gboolean _tmp85_ = FALSE;
		_tmp84_ = self->settings;
		_tmp85_ = g_key_file_get_boolean (_tmp84_, "greeter", "blur", &_inner_error_);
		_tmp83_ = _tmp85_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	__catch2_g_error:
	{
		...
		g_warning ("PantheonGreeter.vala:154: %s", _tmp87_);
		_g_error_free0 (e);
	}
	__finally2:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	_tmp88_ = blur_effect;
	if (_tmp88_) {
		ClutterShaderEffect* shaderEffectVer = NULL;
		ClutterShaderEffect* _tmp89_ = NULL;
		gchar* _tmp90_ = NULL;
		gchar* _tmp91_ = NULL;
		ClutterShaderEffect* _tmp92_ = NULL;
		ClutterShaderEffect* _tmp93_ = NULL;
		GValue _tmp94_ = {0};
		ClutterShaderEffect* _tmp95_ = NULL;
		gint _tmp96_ = 0;
		GValue _tmp97_ = {0};
		ClutterShaderEffect* _tmp98_ = NULL;
		gint _tmp99_ = 0;
		GValue _tmp100_ = {0};
		ClutterShaderEffect* _tmp101_ = NULL;
		GValue _tmp102_ = {0};
		ClutterShaderEffect* _tmp103_ = NULL;
		GValue _tmp104_ = {0};
		GtkClutterActor* _tmp105_ = NULL;
		ClutterShaderEffect* _tmp106_ = NULL;
		_tmp89_ = (ClutterShaderEffect*) clutter_shader_effect_new (CLUTTER_FRAGMENT_SHADER);
		g_object_ref_sink (_tmp89_);
		shaderEffectVer = _tmp89_;
		_tmp91_ = pantheon_greeter_load_from_resource (self, CONSTANTS_PKGDATADIR "/shader.glsl", &_inner_error_);
		_tmp90_ = _tmp91_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			_g_object_unref0 (shaderEffectVer);
			_g_object_unref0 (power_label);
			_g_object_unref0 (time_label);
			_g_object_unref0 (stage);
			_g_free0 (state_file_name);
		...
		_tmp92_ = shaderEffectVer;
		clutter_shader_effect_set_shader_source (_tmp92_, _tmp90_);
		_tmp93_ = shaderEffectVer;
		g_value_init (&_tmp94_, G_TYPE_DOUBLE);
		g_value_set_double (&_tmp94_, 1.0);
		clutter_shader_effect_set_uniform_value (_tmp93_, "dir", &_tmp94_);
		G_IS_VALUE (&_tmp94_) ? (g_value_unset (&_tmp94_), NULL) : NULL;
		_tmp95_ = shaderEffectVer;
		_tmp96_ = self->priv->g_width;
		g_value_init (&_tmp97_, G_TYPE_INT);
		g_value_set_int (&_tmp97_, _tmp96_);
		clutter_shader_effect_set_uniform_value (_tmp95_, "width", &_tmp97_);
		G_IS_VALUE (&_tmp97_) ? (g_value_unset (&_tmp97_), NULL) : NULL;
		_tmp98_ = shaderEffectVer;
		_tmp99_ = self->priv->g_height;
		g_value_init (&_tmp100_, G_TYPE_INT);
		g_value_set_int (&_tmp100_, _tmp99_);
		clutter_shader_effect_set_uniform_value (_tmp98_, "height", &_tmp100_);
		G_IS_VALUE (&_tmp100_) ? (g_value_unset (&_tmp100_), NULL) : NULL;
		_tmp101_ = shaderEffectVer;
		g_value_init (&_tmp102_, G_TYPE_DOUBLE);
		g_value_set_double (&_tmp102_, 10.0);
		clutter_shader_effect_set_uniform_value (_tmp101_, "radius", &_tmp102_);
		G_IS_VALUE (&_tmp102_) ? (g_value_unset (&_tmp102_), NULL) : NULL;
		_tmp103_ = shaderEffectVer;
		g_value_init (&_tmp104_, G_TYPE_DOUBLE);
		g_value_set_double (&_tmp104_, 0.948);
		clutter_shader_effect_set_uniform_value (_tmp103_, "brightness", &_tmp104_);
		G_IS_VALUE (&_tmp104_) ? (g_value_unset (&_tmp104_), NULL) : NULL;
		_tmp105_ = self->priv->wallpaper_actor;
		_tmp106_ = shaderEffectVer;
		clutter_actor_add_effect_with_name ((ClutterActor*) _tmp105_, "blur", (ClutterEffect*) _tmp106_);
		_g_free0 (_tmp90_);
		_g_object_unref0 (shaderEffectVer);
	}
	pantheon_greeter_monitors_changed (self);
	_tmp107_ = clutter_actor_new ();
	g_object_ref_sink (_tmp107_);
	_g_object_unref0 (self->priv->greeterbox);
	self->priv->greeterbox = _tmp107_;
	_tmp108_ = self->priv->greeterbox;
	_tmp109_ = stage;
	_tmp110_ = (ClutterBindConstraint*) clutter_bind_constraint_new ((ClutterActor*) _tmp109_, CLUTTER_BIND_WIDTH, (gfloat) 0);
	g_object_ref_sink (_tmp110_);
	_tmp111_ = _tmp110_;
	clutter_actor_add_constraint (_tmp108_, (ClutterConstraint*) _tmp111_);
	_g_object_unref0 (_tmp111_);
	_tmp112_ = self->priv->greeterbox;
	_tmp113_ = stage;
	_tmp114_ = (ClutterBindConstraint*) clutter_bind_constraint_new ((ClutterActor*) _tmp113_, CLUTTER_BIND_HEIGHT, (gfloat) 0);
	g_object_ref_sink (_tmp114_);
	_tmp115_ = _tmp114_;
	clutter_actor_add_constraint (_tmp112_, (ClutterConstraint*) _tmp115_);
	_g_object_unref0 (_tmp115_);
	_tmp116_ = self->priv->greeterbox;
	clutter_actor_set_opacity (_tmp116_, (guint) 0);
	_tmp117_ = self->priv->greeterbox;
	clutter_actor_save_easing_state (_tmp117_);
	_tmp118_ = self->priv->greeterbox;
	clutter_actor_set_easing_mode (_tmp118_, CLUTTER_EASE_OUT_QUART);
	_tmp119_ = self->priv->greeterbox;
	clutter_actor_set_easing_duration (_tmp119_, (guint) 250);
	_tmp120_ = self->priv->greeterbox;
	clutter_actor_set_opacity (_tmp120_, (guint8) 255);
	_tmp121_ = self->priv->greeterbox;
	clutter_actor_restore_easing_state (_tmp121_);
	_tmp122_ = self->priv->greeterbox;
	_tmp123_ = self->priv->wallpaper_actor;
	clutter_actor_add_child (_tmp122_, (ClutterActor*) _tmp123_);
	_tmp124_ = self->priv->greeterbox;
	_tmp125_ = self->priv->power_actor;
	clutter_actor_add_child (_tmp124_, (ClutterActor*) _tmp125_);
	_tmp126_ = self->priv->greeterbox;
	_tmp127_ = self->priv->time_actor;
	clutter_actor_add_child (_tmp126_, (ClutterActor*) _tmp127_);
	_tmp128_ = self->priv->greeterbox;
	_tmp129_ = self->priv->userlist_actor;
	clutter_actor_add_child (_tmp128_, (ClutterActor*) _tmp129_);
	_tmp130_ = stage;
	_tmp131_ = self->priv->greeterbox;
	clutter_actor_add_child ((ClutterActor*) _tmp130_, _tmp131_);
	_tmp132_ = self->priv->clutter;
	gtk_container_add ((GtkContainer*) self, (GtkWidget*) _tmp132_);
	activate_numlock = FALSE;
	{
		gboolean _tmp133_ = FALSE;
		GKeyFile* _tmp134_ = NULL;
		gboolean _tmp135_ = FALSE;
		_tmp134_ = self->settings;
		_tmp135_ = g_key_file_get_boolean (_tmp134_, "greeter", "activate-numlock", &_inner_error_);
		_tmp133_ = _tmp135_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			goto __catch3_g_error;
		}
		activate_numlock = _tmp133_;
	}
	goto __finally3;
	__catch3_g_error:
	{
		...
		g_warning ("PantheonGreeter.vala:200: %s", _tmp137_);
		_g_error_free0 (e);
	}
	__finally3:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	screensaver_timeout = 60;
	{
		...
		gint _tmp141_ = 0;
		_tmp140_ = self->settings;
		_tmp141_ = g_key_file_get_integer (_tmp140_, "greeter", "screensaver-timeout", &_inner_error_);
		_tmp139_ = _tmp141_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			goto __catch4_g_error;
		}
		screensaver_timeout = _tmp139_;
	}
	goto __finally4;
	__catch4_g_error:
	{
		...
		g_warning ("PantheonGreeter.vala:210: %s", _tmp143_);
		_g_error_free0 (e);
	}
	__finally4:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	_tmp144_ = gdk_display_get_default ();
	_tmp145_ = gdk_x11_display_get_xdisplay (G_TYPE_CHECK_INSTANCE_TYPE (_tmp144_, gdk_x11_display_get_type ()) ? ((GdkX11Display*) _tmp144_) : NULL);
	display = _tmp145_;
	_tmp146_ = display;
	XGetScreenSaver (_tmp146_, &_tmp147_, &_tmp148_, &_tmp149_, &_tmp150_);
	self->priv->timeout = _tmp147_;
	self->priv->interval = _tmp148_;
	self->priv->prefer_blanking = _tmp149_;
	self->priv->allow_exposures = _tmp150_;
	_tmp151_ = display;
	_tmp152_ = screensaver_timeout;
	XSetScreenSaver (_tmp151_, _tmp152_, 0, (gint) PANTHEON_GREETER_SCREENSAVER_ACTIVE, (gint) PANTHEON_GREETER_EXPOSURES_DEFAULT_EXPOSURES);
	_tmp153_ = pantheon_greeter_get_login_gateway ();
	_tmp154_ = _tmp153_;
	_tmp155_ = login_gateway_get_lock (_tmp154_);
	_tmp156_ = _tmp155_;
	if (_tmp156_) {
		Display* _tmp157_ = NULL;
		_tmp157_ = display;
		XForceScreenSaver (_tmp157_, (gint) PANTHEON_GREETER_SCREENSAVER_ACTIVE);
	}
	pantheon_greeter_connect_signals (self);
	_tmp158_ = pantheon_greeter_get_login_gateway ();
	_tmp159_ = _tmp158_;
	_tmp160_ = login_gateway_get_select_user (_tmp159_);
	_tmp161_ = _tmp160_;
	_tmp162_ = g_strdup (_tmp161_);
	select_user = _tmp162_;
	_tmp164_ = select_user;
	if (_tmp164_ != NULL) {
		const gchar* _tmp165_ = NULL;
		gchar* _tmp166_ = NULL;
		_tmp165_ = select_user;
		_tmp166_ = g_strdup (_tmp165_);
		_g_free0 (_tmp163_);
		_tmp163_ = _tmp166_;
	} else {
		gchar* _tmp167_ = NULL;
		_tmp167_ = pantheon_greeter_get_greeter_state (self, "last-user");
		_g_free0 (_tmp163_);
		_tmp163_ = _tmp167_;
	}
	_tmp168_ = g_strdup (_tmp163_);
	switch_to_user = _tmp168_;
	_tmp169_ = switch_to_user;
	if (_tmp169_ != NULL) {
		{
			gint i = 0;
			i = 0;
			{
				...
				while (TRUE) {
					...
					_tmp170_ = FALSE;
					_tmp172_ = i;
					_tmp173_ = self->priv->userlist;
					_tmp174_ = user_list_get_size (_tmp173_);
					_tmp175_ = _tmp174_;
					if (!(_tmp172_ < _tmp175_)) {
						break;
					}
					...
					_tmp179_ = _tmp178_;
					_tmp180_ = login_option_get_name (_tmp179_);
					_tmp181_ = _tmp180_;
					_tmp182_ = switch_to_user;
					_tmp183_ = g_strcmp0 (_tmp181_, _tmp182_) == 0;
					_g_object_unref0 (_tmp179_);
					if (_tmp183_) {
						UserList* _tmp184_ = NULL;
						UserList* _tmp185_ = NULL;
						gint _tmp186_ = 0;
						LoginOption* _tmp187_ = NULL;
						LoginOption* _tmp188_ = NULL;
						_tmp184_ = self->priv->userlist;
						_tmp185_ = self->priv->userlist;
						_tmp186_ = i;
						_tmp187_ = user_list_get_user (_tmp185_, _tmp186_);
						_tmp188_ = _tmp187_;
						user_list_set_current_user (_tmp184_, _tmp188_);
	...
	_tmp189_ = self->priv->userlist;
	_tmp190_ = user_list_get_current_user (_tmp189_);
	_tmp191_ = _tmp190_;
	if (_tmp191_ == NULL) {
		UserList* _tmp192_ = NULL;
		UserList* _tmp193_ = NULL;
		LoginOption* _tmp194_ = NULL;
		LoginOption* _tmp195_ = NULL;
		_tmp192_ = self->priv->userlist;
		_tmp193_ = self->priv->userlist;
		_tmp194_ = user_list_get_user (_tmp193_, 0);
		_tmp195_ = _tmp194_;
		user_list_set_current_user (_tmp192_, _tmp195_);
		_g_object_unref0 (_tmp195_);
	}
	_tmp196_ = gtk_widget_get_style_context ((GtkWidget*) self);
	gtk_style_context_add_class (_tmp196_, "greeter");
	_tmp197_ = gtk_css_provider_new ();
	provider = _tmp197_;
	_tmp198_ = provider;
	gtk_css_provider_load_from_resource (_tmp198_, "io/elementary/greeter/Greeter.css");
	_tmp199_ = gdk_screen_get_default ();
	_tmp200_ = provider;
	gtk_style_context_add_provider_for_screen (_tmp199_, (GtkStyleProvider*) _tmp200_, (guint) GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
	gtk_widget_show_all ((GtkWidget*) self);
	_tmp201_ = self->priv->userlist_actor;
	_tmp202_ = pantheon_greeter_fade_in_actor (self, (ClutterActor*) _tmp201_);
	_tmp203_ = _tmp202_;
	_g_object_unref0 (_tmp203_);
	_tmp204_ = self->priv->time_actor;
	_tmp205_ = pantheon_greeter_fade_in_actor (self, (ClutterActor*) _tmp204_);
	_tmp206_ = _tmp205_;
	_g_object_unref0 (_tmp206_);
	_tmp207_ = self->priv->wallpaper_actor;
	_tmp208_ = pantheon_greeter_fade_in_actor (self, (ClutterActor*) _tmp207_);
	_tmp209_ = _tmp208_;
	_g_object_unref0 (_tmp209_);
	_tmp210_ = gtk_widget_get_window ((GtkWidget*) self);
	gdk_window_focus (_tmp210_, (guint32) GDK_CURRENT_TIME);
...
static void __lambda27_ (PantheonGreeter* self) {
	...
	g_message ("PantheonGreeter.vala:274: restore user timeout: %d", _tmp2_);
...
static ClutterPropertyTransition* pantheon_greeter_fade_out_actor (PantheonGreeter* self, ClutterActor* actor) {
	...
	clutter_timeline_set_duration ((ClutterTimeline*) transition, (guint) 300);
	...
	switch (_tmp1_) {
		case GDK_KEY_Num_Lock:
		{
			...
			__catch5_g_error:
			{
				...
				g_warning ("PantheonGreeter.vala:395: %s", _tmp6_);
				_g_error_free0 (e);
			}
			__finally5:
			if (G_UNLIKELY (_inner_error_ != NULL)) {
...
void pantheon_greeter_set_greeter_state (PantheonGreeter* self, const gchar* key, const gchar* value) {
	...
	__catch7_g_error:
	{
		...
		g_warning ("PantheonGreeter.vala:441: Failed to write state: %s", _tmp7_);
		_g_error_free0 (e);
	}
	__finally7:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
...
gint _vala_main (gchar** args, int args_length1) {
	...
	if (init != CLUTTER_INIT_SUCCESS) {
		g_error ("PantheonGreeter.vala:526: Clutter could not be intiailized");
	}
...
gchar* get_defaults (const gchar* _default_) {
	...
	__catch8_g_error:
	{
		...
		g_warning ("PantheonGreeter.vala:573: %s", _tmp24_);
		_g_error_free0 (e);
	}
	__finally8:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
