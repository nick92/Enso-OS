...
PantheonGreeter* pantheon_greeter_construct (GType object_type) {
	...
	GtkClutterActor* _tmp100_ = NULL;
	ClutterActor* _tmp101_ = NULL;
	UserListActor* _tmp102_ = NULL;
	ClutterActor* _tmp103_ = NULL;
	GtkClutterEmbed* _tmp104_ = NULL;
	gboolean activate_numlock = FALSE;
	gboolean _tmp110_ = FALSE;
	gint screensaver_timeout = 0;
	Display* display = NULL;
	GdkDisplay* _tmp116_ = NULL;
	Display* _tmp117_ = NULL;
	Display* _tmp118_ = NULL;
	gint _tmp119_ = 0;
	gint _tmp120_ = 0;
	gint _tmp121_ = 0;
	gint _tmp122_ = 0;
	Display* _tmp123_ = NULL;
	gint _tmp124_ = 0;
	LoginGateway* _tmp125_ = NULL;
	LoginGateway* _tmp126_ = NULL;
	gboolean _tmp127_ = FALSE;
	gboolean _tmp128_ = FALSE;
	gchar* select_user = NULL;
	LoginGateway* _tmp130_ = NULL;
	LoginGateway* _tmp131_ = NULL;
	...
	UserList* _tmp161_ = NULL;
	LoginOption* _tmp162_ = NULL;
	LoginOption* _tmp163_ = NULL;
	GtkStyleContext* _tmp168_ = NULL;
	GtkCssProvider* provider = NULL;
	GtkCssProvider* _tmp169_ = NULL;
	GdkScreen* _tmp170_ = NULL;
	UserListActor* _tmp171_ = NULL;
	ClutterPropertyTransition* _tmp172_ = NULL;
	ClutterPropertyTransition* _tmp173_ = NULL;
	GtkClutterActor* _tmp174_ = NULL;
	ClutterPropertyTransition* _tmp175_ = NULL;
	ClutterPropertyTransition* _tmp176_ = NULL;
	GtkClutterActor* _tmp177_ = NULL;
	ClutterPropertyTransition* _tmp178_ = NULL;
	ClutterPropertyTransition* _tmp179_ = NULL;
	GdkWindow* _tmp180_ = NULL;
	...
	if (_tmp17_) {
		...
		g_message ("PantheonGreeter.vala:89: Using dummy LightDM because LIGHTDM_TO_SERVER" \
"_FD was not found.");
	...
	__catch0_g_error:
	{
		...
		if (!g_error_matches (_tmp46_, G_FILE_ERROR, G_FILE_ERROR_NOENT)) {
			...
			g_warning ("PantheonGreeter.vala:109: Failed to load state from %s: %s\n", _tmp47_, _tmp49_);
		}
		_g_error_free0 (e);
	}
	__finally0:
	...
	__catch1_g_error:
	{
		...
		g_warning ("PantheonGreeter.vala:117: %s", _tmp55_);
		_g_error_free0 (e);
	}
	__finally1:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	clutter_actor_set_opacity ((ClutterActor*) _tmp71_, (guint8) 0);
	_tmp72_ = power_label_new ();
	g_object_ref_sink (_tmp72_);
	power_label = _tmp72_;
	_tmp73_ = (GtkClutterActor*) gtk_clutter_actor_new ();
	g_object_ref_sink (_tmp73_);
	_g_object_unref0 (self->priv->power_actor);
	self->priv->power_actor = _tmp73_;
	_tmp74_ = self->priv->power_actor;
	_tmp75_ = gtk_clutter_actor_get_widget (_tmp74_);
	gtk_container_add (G_TYPE_CHECK_INSTANCE_CAST (_tmp75_, gtk_container_get_type (), GtkContainer), (GtkWidget*) power_label);
	_tmp76_ = self->priv->power_actor;
	clutter_actor_set_opacity ((ClutterActor*) _tmp76_, (guint8) 0);
	_tmp77_ = wallpaper_new ();
	g_object_ref_sink (_tmp77_);
	_g_object_unref0 (self->priv->wallpaper);
	self->priv->wallpaper = _tmp77_;
	_tmp78_ = (GtkClutterActor*) gtk_clutter_actor_new ();
	g_object_ref_sink (_tmp78_);
	_g_object_unref0 (self->priv->wallpaper_actor);
	self->priv->wallpaper_actor = _tmp78_;
	_tmp79_ = self->priv->wallpaper_actor;
	_tmp80_ = gtk_clutter_actor_get_widget (_tmp79_);
	_tmp81_ = self->priv->wallpaper;
	gtk_container_add (G_TYPE_CHECK_INSTANCE_CAST (_tmp80_, gtk_container_get_type (), GtkContainer), (GtkWidget*) _tmp81_);
	pantheon_greeter_monitors_changed (self);
	_tmp82_ = clutter_actor_new ();
	g_object_ref_sink (_tmp82_);
	_g_object_unref0 (self->priv->greeterbox);
	self->priv->greeterbox = _tmp82_;
	_tmp83_ = self->priv->greeterbox;
	_tmp84_ = (ClutterBindConstraint*) clutter_bind_constraint_new ((ClutterActor*) stage, CLUTTER_BIND_WIDTH, (gfloat) 0);
	g_object_ref_sink (_tmp84_);
	_tmp85_ = _tmp84_;
	clutter_actor_add_constraint (_tmp83_, (ClutterConstraint*) _tmp85_);
	_g_object_unref0 (_tmp85_);
	_tmp86_ = self->priv->greeterbox;
	_tmp87_ = (ClutterBindConstraint*) clutter_bind_constraint_new ((ClutterActor*) stage, CLUTTER_BIND_HEIGHT, (gfloat) 0);
	g_object_ref_sink (_tmp87_);
	_tmp88_ = _tmp87_;
	clutter_actor_add_constraint (_tmp86_, (ClutterConstraint*) _tmp88_);
	_g_object_unref0 (_tmp88_);
	_tmp89_ = self->priv->greeterbox;
	clutter_actor_set_opacity (_tmp89_, (guint) 0);
	_tmp90_ = self->priv->greeterbox;
	clutter_actor_save_easing_state (_tmp90_);
	_tmp91_ = self->priv->greeterbox;
	clutter_actor_set_easing_mode (_tmp91_, CLUTTER_EASE_OUT_QUART);
	_tmp92_ = self->priv->greeterbox;
	clutter_actor_set_easing_duration (_tmp92_, (guint) 250);
	_tmp93_ = self->priv->greeterbox;
	clutter_actor_set_opacity (_tmp93_, (guint8) 255);
	_tmp94_ = self->priv->greeterbox;
	clutter_actor_restore_easing_state (_tmp94_);
	_tmp95_ = self->priv->greeterbox;
	_tmp96_ = self->priv->wallpaper_actor;
	clutter_actor_add_child (_tmp95_, (ClutterActor*) _tmp96_);
	_tmp97_ = self->priv->greeterbox;
	_tmp98_ = self->priv->power_actor;
	clutter_actor_add_child (_tmp97_, (ClutterActor*) _tmp98_);
	_tmp99_ = self->priv->greeterbox;
	_tmp100_ = self->priv->time_actor;
	clutter_actor_add_child (_tmp99_, (ClutterActor*) _tmp100_);
	_tmp101_ = self->priv->greeterbox;
	_tmp102_ = self->priv->userlist_actor;
	clutter_actor_add_child (_tmp101_, (ClutterActor*) _tmp102_);
	_tmp103_ = self->priv->greeterbox;
	clutter_actor_add_child ((ClutterActor*) stage, _tmp103_);
	_tmp104_ = self->priv->clutter;
	...
	activate_numlock = FALSE;
	{
		...
		_tmp106_ = self->settings;
		_tmp107_ = g_key_file_get_boolean (_tmp106_, "greeter", "activate-numlock", &_inner_error_);
		_tmp105_ = _tmp107_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			goto __catch2_g_error;
		}
		activate_numlock = _tmp105_;
	}
	goto __finally2;
	__catch2_g_error:
	{
		...
		g_warning ("PantheonGreeter.vala:175: %s", _tmp109_);
		_g_error_free0 (e);
	}
	__finally2:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	screensaver_timeout = 60;
	{
		...
		gint _tmp113_ = 0;
		_tmp112_ = self->settings;
		_tmp113_ = g_key_file_get_integer (_tmp112_, "greeter", "screensaver-timeout", &_inner_error_);
		_tmp111_ = _tmp113_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			goto __catch3_g_error;
		}
		screensaver_timeout = _tmp111_;
	}
	goto __finally3;
	__catch3_g_error:
	{
		...
		g_warning ("PantheonGreeter.vala:185: %s", _tmp115_);
		_g_error_free0 (e);
	}
	__finally3:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	_tmp116_ = gdk_display_get_default ();
	_tmp117_ = gdk_x11_display_get_xdisplay (G_TYPE_CHECK_INSTANCE_TYPE (_tmp116_, gdk_x11_display_get_type ()) ? ((GdkX11Display*) _tmp116_) : NULL);
	display = _tmp117_;
	_tmp118_ = display;
	XGetScreenSaver (_tmp118_, &_tmp119_, &_tmp120_, &_tmp121_, &_tmp122_);
	self->priv->timeout = _tmp119_;
	self->priv->interval = _tmp120_;
	self->priv->prefer_blanking = _tmp121_;
	self->priv->allow_exposures = _tmp122_;
	_tmp123_ = display;
	_tmp124_ = screensaver_timeout;
	XSetScreenSaver (_tmp123_, _tmp124_, 0, (gint) PANTHEON_GREETER_SCREENSAVER_ACTIVE, (gint) PANTHEON_GREETER_EXPOSURES_DEFAULT_EXPOSURES);
	_tmp125_ = pantheon_greeter_get_login_gateway ();
	_tmp126_ = _tmp125_;
	_tmp127_ = login_gateway_get_lock (_tmp126_);
	_tmp128_ = _tmp127_;
	if (_tmp128_) {
		Display* _tmp129_ = NULL;
		_tmp129_ = display;
		XForceScreenSaver (_tmp129_, (gint) PANTHEON_GREETER_SCREENSAVER_ACTIVE);
	}
	pantheon_greeter_connect_signals (self);
	_tmp130_ = pantheon_greeter_get_login_gateway ();
	_tmp131_ = _tmp130_;
	_tmp132_ = login_gateway_get_select_user (_tmp131_);
	_tmp133_ = _tmp132_;
	_tmp134_ = g_strdup (_tmp133_);
	select_user = _tmp134_;
	_tmp136_ = select_user;
	if (_tmp136_ != NULL) {
		const gchar* _tmp137_ = NULL;
		gchar* _tmp138_ = NULL;
		_tmp137_ = select_user;
		_tmp138_ = g_strdup (_tmp137_);
		_g_free0 (_tmp135_);
		_tmp135_ = _tmp138_;
	} else {
		gchar* _tmp139_ = NULL;
		_tmp139_ = pantheon_greeter_get_greeter_state (self, "last-user");
		_g_free0 (_tmp135_);
		_tmp135_ = _tmp139_;
	}
	_tmp140_ = g_strdup (_tmp135_);
	switch_to_user = _tmp140_;
	_tmp141_ = switch_to_user;
	if (_tmp141_ != NULL) {
		{
			gint i = 0;
			i = 0;
			{
				...
				while (TRUE) {
					gint _tmp144_ = 0;
					UserList* _tmp145_ = NULL;
					gint _tmp146_ = 0;
					gint _tmp147_ = 0;
					UserList* _tmp148_ = NULL;
					gint _tmp149_ = 0;
					LoginOption* _tmp150_ = NULL;
					LoginOption* _tmp151_ = NULL;
					...
					_tmp142_ = FALSE;
					_tmp144_ = i;
					_tmp145_ = self->priv->userlist;
					_tmp146_ = user_list_get_size (_tmp145_);
					_tmp147_ = _tmp146_;
					if (!(_tmp144_ < _tmp147_)) {
						break;
					}
					_tmp148_ = self->priv->userlist;
					_tmp149_ = i;
					_tmp150_ = user_list_get_user (_tmp148_, _tmp149_);
					_tmp151_ = _tmp150_;
					_tmp152_ = login_option_get_name (_tmp151_);
					_tmp153_ = _tmp152_;
					_tmp154_ = switch_to_user;
					_tmp155_ = g_strcmp0 (_tmp153_, _tmp154_) == 0;
					_g_object_unref0 (_tmp151_);
					if (_tmp155_) {
						UserList* _tmp156_ = NULL;
						UserList* _tmp157_ = NULL;
						gint _tmp158_ = 0;
						LoginOption* _tmp159_ = NULL;
						LoginOption* _tmp160_ = NULL;
						_tmp156_ = self->priv->userlist;
						_tmp157_ = self->priv->userlist;
						_tmp158_ = i;
						_tmp159_ = user_list_get_user (_tmp157_, _tmp158_);
						_tmp160_ = _tmp159_;
						user_list_set_current_user (_tmp156_, _tmp160_);
	...
	_tmp161_ = self->priv->userlist;
	_tmp162_ = user_list_get_current_user (_tmp161_);
	_tmp163_ = _tmp162_;
	if (_tmp163_ == NULL) {
		UserList* _tmp164_ = NULL;
		UserList* _tmp165_ = NULL;
		LoginOption* _tmp166_ = NULL;
		LoginOption* _tmp167_ = NULL;
		_tmp164_ = self->priv->userlist;
		_tmp165_ = self->priv->userlist;
		_tmp166_ = user_list_get_user (_tmp165_, 0);
		_tmp167_ = _tmp166_;
		user_list_set_current_user (_tmp164_, _tmp167_);
		_g_object_unref0 (_tmp167_);
	}
	_tmp168_ = gtk_widget_get_style_context ((GtkWidget*) self);
	gtk_style_context_add_class (_tmp168_, "greeter");
	_tmp169_ = gtk_css_provider_new ();
	provider = _tmp169_;
	gtk_css_provider_load_from_resource (provider, "io/elementary/greeter/Greeter.css");
	_tmp170_ = gdk_screen_get_default ();
	gtk_style_context_add_provider_for_screen (_tmp170_, (GtkStyleProvider*) provider, (guint) GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
	gtk_widget_show_all ((GtkWidget*) self);
	_tmp171_ = self->priv->userlist_actor;
	_tmp172_ = pantheon_greeter_fade_in_actor (self, (ClutterActor*) _tmp171_);
	_tmp173_ = _tmp172_;
	_g_object_unref0 (_tmp173_);
	_tmp174_ = self->priv->time_actor;
	_tmp175_ = pantheon_greeter_fade_in_actor (self, (ClutterActor*) _tmp174_);
	_tmp176_ = _tmp175_;
	_g_object_unref0 (_tmp176_);
	_tmp177_ = self->priv->power_actor;
	_tmp178_ = pantheon_greeter_fade_out_actor (self, (ClutterActor*) _tmp177_);
	_tmp179_ = _tmp178_;
	_g_object_unref0 (_tmp179_);
	_tmp180_ = gtk_widget_get_window ((GtkWidget*) self);
	gdk_window_focus (_tmp180_, (guint32) GDK_CURRENT_TIME);
...
static void __lambda27_ (PantheonGreeter* self) {
	...
	g_message ("PantheonGreeter.vala:237: restore user timeout: %d", _tmp2_);
...
static void pantheon_greeter_reposition (PantheonGreeter* self) {
	...
	gint _tmp27_ = 0;
	Wallpaper* _tmp28_ = NULL;
	gint _tmp29_ = 0;
	Wallpaper* _tmp30_ = NULL;
	gint _tmp31_ = 0;
	Wallpaper* _tmp32_ = NULL;
	g_return_if_fail (self != NULL);
	width = 0;
	height = 0;
	...
	_tmp13_ = width;
	clutter_actor_set_x ((ClutterActor*) _tmp12_, (gfloat) ((_tmp13_ - PANTHEON_GREETER_DEFAULT_CLOCK_WIDTH) - 10));
	_tmp14_ = self->priv->time_actor;
	_tmp15_ = height;
	clutter_actor_set_y ((ClutterActor*) _tmp14_, (gfloat) (_tmp15_ - PANTHEON_GREETER_DEFAULT_CLOCK_HEIGHT));
	_tmp16_ = self->priv->time_actor;
	_tmp17_ = width;
	g_object_set ((ClutterActor*) _tmp16_, "visible", _tmp17_ > PANTHEON_GREETER_NO_CLOCK_WIDTH, NULL);
	_tmp18_ = self->priv->power_actor;
	_tmp19_ = width;
	_tmp20_ = self->priv->power_actor;
	_tmp21_ = clutter_actor_get_width ((ClutterActor*) _tmp20_);
	_tmp22_ = _tmp21_;
	clutter_actor_set_x ((ClutterActor*) _tmp18_, (_tmp19_ - _tmp22_) - 10);
	_tmp23_ = self->priv->power_actor;
	clutter_actor_set_y ((ClutterActor*) _tmp23_, (gfloat) 10);
	_tmp24_ = self->priv->wallpaper_actor;
	_tmp25_ = width;
	clutter_actor_set_width ((ClutterActor*) _tmp24_, (gfloat) _tmp25_);
	_tmp26_ = self->priv->wallpaper_actor;
	_tmp27_ = height;
	clutter_actor_set_height ((ClutterActor*) _tmp26_, (gfloat) _tmp27_);
	_tmp28_ = self->priv->wallpaper;
	_tmp29_ = width;
	wallpaper_set_screen_width (_tmp28_, _tmp29_);
	_tmp30_ = self->priv->wallpaper;
	_tmp31_ = height;
	wallpaper_set_screen_height (_tmp30_, _tmp31_);
	_tmp32_ = self->priv->wallpaper;
	wallpaper_reposition (_tmp32_);
}
	...
	switch (_tmp1_) {
		case GDK_KEY_Num_Lock:
		{
			...
			__catch4_g_error:
			{
				...
				g_warning ("PantheonGreeter.vala:354: %s", _tmp6_);
				_g_error_free0 (e);
			}
			__finally4:
			if (G_UNLIKELY (_inner_error_ != NULL)) {
...
void pantheon_greeter_set_greeter_state (PantheonGreeter* self, const gchar* key, const gchar* value) {
	...
	__catch6_g_error:
	{
		...
		g_warning ("PantheonGreeter.vala:400: Failed to write state: %s", _tmp7_);
		_g_error_free0 (e);
	}
	__finally6:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
...
gint _vala_main (gchar** args, int args_length1) {
	...
	if (init != CLUTTER_INIT_SUCCESS) {
		g_error ("PantheonGreeter.vala:485: Clutter could not be intiailized");
	}
...
gchar* get_defaults (const gchar* _default_) {
	...
	__catch7_g_error:
	{
		...
		g_warning ("PantheonGreeter.vala:529: %s", _tmp20_);
		_g_error_free0 (e);
	}
	__finally7:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
